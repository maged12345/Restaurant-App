...
//  Restaurant App
//
//  Created by majid mohmed on 18/10/2021.
...
class AboutTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
     ...
     let cell = tableView.dequeueReusableCell(withIdentifier: "AboutCell", for: indexPath)
        ...
        let sectionData = sectionContent[indexPath.section][indexPath.row]
        
        cell.textLabel?.text = sectionData.text
        cell.imageView?.image = UIImage(named: sectionData.image )
     ...
     */
     ...
     override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier ==  "ShowWebView" {
            if let des = segue.destination as? WebViewViewController , let indexpath = tableView.indexPathForSelectedRow{
                des.linkPassed = sectionContent[indexpath.section][indexpath.row].link
            }
        }
